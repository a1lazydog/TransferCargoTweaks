...
if onClient() then
    ...
    function TransferCrewGoods.checkPermissionsAndDistance(player, source, target) -- overridden
        ...
        if TCTConfig.RequireAlliancePrivileges then
            ...
            if (source.allianceOwned and source.isShip) or (target.allianceOwned and target.isShip) then
                requiredPrivileges[#requiredPrivileges+1] = AlliancePrivilege.ManageShips
            end
            if (source.allianceOwned and source.isStation) or (target.allianceOwned and target.isStation) then
        ...
        -- check distance
        local transferDistance = math.max(TCTConfig.CrewMaxTransferDistance, source.transporterRange or 0, target.transporterRange or 0)
        if TCTConfig.CheckIfDocked and (source.isStation or target.isStation) then
            if ((source.isStation and not source:isDocked(target)) or (target.isStation and not target:isDocked(source)))
            and source:getNearestDistance(target) > transferDistance then
                player:sendChatMessage("", ChatMessageType.Error, "You must be docked to the station to transfer crew."%_t)
                return
            end
        elseif source:getNearestDistance(target) > transferDistance then
